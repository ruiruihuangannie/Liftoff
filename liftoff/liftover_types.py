...
def lift_original_annotation(ref_chroms, target_chroms, lifted_features_list, args, unmapped_features, feature_db, 
                             feature_hierarchy, ref_parent_order, feature_type):
...
def align_and_lift_features(ref_chroms, target_chroms, args, feature_hierarchy, liftover_type, unmapped_features, 
    ...
    aligned_segments= align_features.align_features_to_target(ref_chroms, target_chroms, args, feature_hierarchy, 
                                                              liftover_type, unmapped_features, feature_type)
    print("[Info]: Lifting features.")
    feature_locations = None
    lift_features.lift_all_features(aligned_segments, min_cov, feature_db,  feature_hierarchy, unmapped_features, 
                                    lifted_features_list, min_seqid, feature_locations, args, ref_parent_order, 
                                    feature_type)
    fix_overlapping_features.fix_incorrectly_overlapping_features(lifted_features_list, lifted_features_list, 
                                    aligned_segments, unmapped_features, min_cov, feature_hierarchy, feature_db, 
                                    ref_parent_order, min_seqid, args, max_overlap, feature_type)


def map_unmapped_genes_agaisnt_all(unmapped_features, ref_chroms, target_chroms, lifted_features_list, feature_db, 
                                   feature_hierarchy, ref_parent_order, args, feature_type):
...
def map_unplaced_genes(unmapped_features, ref_chroms, target_chroms, lifted_features_list, feature_db, 
                       feature_hierarchy, ref_parent_order, args, feature_type):
    liftoff_utils.clear_scores(lifted_features_list, feature_hierarchy.parents)
    liftover_type = "unplaced"
    unplaced_dict = get_features_from_unplaced_seq(ref_chroms, feature_hierarchy)
    extract_features.get_gene_sequences(unplaced_dict, ref_chroms, args, liftover_type, feature_type)
    if args.exclude_partial:
...
def map_extra_copies(ref_chroms, target_chroms, lifted_features_list, feature_hierarchy, feature_db, 
                     ref_parent_order, args, feature_type):
    liftoff_utils.clear_scores(lifted_features_list, feature_hierarchy.parents)
    unmapped_features = []
    extract_features.get_gene_sequences(feature_hierarchy.parents, ref_chroms, args, "copies", feature_type)
    min_cov, min_seqid = 0, args.sc
    align_and_lift_features(ref_chroms, target_chroms, args, feature_hierarchy, "copies", unmapped_features, 
                            feature_db, lifted_features_list, ref_parent_order, min_cov, min_seqid, args.overlap, 
                            feature_type)
